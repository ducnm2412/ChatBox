...
from flask_socketio import SocketIO, emit, disconnect, join_room  # ✅ FIX: thêm join_room
...
def send_friend_request():
    ...
    if existing:
        return jsonify({'error': 'Relationship exists'}), 400

    new_friendship = Friendship(sender_id=sender_id, receiver_id=receiver_id, status='pending')
    db.session.add(new_friendship)
    db.session.commit()

    # ✅ FIX: gửi theo room=user_id thay vì sid
    socketio.emit('new_friend_request', {'from_user': sender_id}, room=str(receiver_id))
...
def handle_connect(auth):
    ...
    # ✅ FIX: join room theo user_id (ổn định cho multi client/reconnect)
    join_room(str(user_id))

    # giữ map SID để debug/compat (không xóa chức năng cũ)
    user_to_sid[user_id] = request.sid
    sid_to_user[request.sid] = user_id

    print(f"User {user.id} connected with SID {request.sid} and joined room {user.id}")
...
def handle_send_message(data):
    ...
    # ✅ FIX: gửi theo room=user_id (không phụ thuộc sid)
    socketio.emit('new_message', payload, room=str(receiver_id))
    socketio.emit('new_message', payload, room=str(sender_id))
