...
class ChatClient:
    ...
    def on_new_message(self, data): 
        self.message_queue.put(('new_message', data))

    def on_friend_request(self, data): 
        self.message_queue.put(('new_request', data))
...
class ChatApp(ctk.CTk):
    def __init__(self, client_logic):
        ...
        # ✅ SỬA: bind phải gọi đúng method tồn tại trong class
        self.entry_msg.bind("<Return>", self.send_msg)

        ctk.CTkButton(self.input_bar, text="📷", width=45, height=45, corner_radius=25,
             fg_color="#9AA0A6", command=self.send_image).pack(side="right", padx=(0,10))

        # ✅ SỬA: command gọi đúng method
        ctk.CTkButton(self.input_bar, text="➤", width=45, height=45, corner_radius=25,
             fg_color=COLOR_ACCENT, command=self.send_msg).pack(side="right")
    ...
    def on_search(self, event=None):
        ...
        if not res: 
            ctk.CTkLabel(self.list_scroll, text="No users found", text_color="gray").pack(pady=20)
        for u in res:
            ...
            st = u['status']
            if st == 'none': 
                ctk.CTkButton(f, text="Add", width=60, height=25, command=lambda i=u['id']: self.req(i)).pack(side="right", padx=10)
            elif st == 'accepted': 
                ctk.CTkLabel(f, text="Friend", text_color="green").pack(side="right", padx=10)
            elif st == 'pending': 
                ctk.CTkLabel(f, text="Sent", text_color="gray").pack(side="right", padx=10)
            elif st == 'incoming_request': 
                ctk.CTkLabel(f, text="Pending", text_color="orange").pack(side="right", padx=10)
    ...
    def refresh_sidebar(self):
        ...
        if not friends: 
            ctk.CTkLabel(self.list_scroll, text="No friends yet", text_color="gray").pack(pady=20)
        for f in friends:
    ...
    def open_chat(self, uid, uname):
        ...
        for w in self.header_avt_frame.winfo_children():
            w.destroy()
    ...
    def send_image(self):
        if not self.current_pid:
            ...
            return
        if not self.client.connected:   # ✅ THÊM
            messagebox.showwarning("Warning", "Connecting to server, please wait...")
            return
        ...
        try:
            ...
            self.client.sio.emit("send_message", {
                "to_user_id": self.current_pid,
                "content": b64,
                "type": "image",
                "file_name": path.split("\\")[-1]
            })
        except Exception as e:
            messagebox.showerror("Error", f"Cannot send image: {e}")
    ...
    def add_bubble(self, m):
        ...
        if msg_type == "image":
            try:
                ...
                cimg = ctk.CTkImage(light_image=pil_img, size=pil_img.size)
                ...
                lbl.image = cimg
                lbl.pack(anchor=anchor, padx=20)

                self.after(10, self.scroll_btm)
                return
            except:
                ChatBubble(self.msg_scroll, "[Image error]", is_me, m.get("timestamp", "")).pack(fill="x", pady=5)
                self.after(10, self.scroll_btm)
                return
    ...
    def process_queue(self):
        try:
            while not self.client.message_queue.empty():
                ...
                if t == 'new_message':
                    if self.current_pid and (int(d['sender_id']) == int(self.current_pid) or int(d['receiver_id']) == int(self.current_pid)):
                        self.add_bubble(d)
                elif t == 'new_request':
                    if self.mode == "friends":
                        self.refresh_sidebar()
...
COLOR_BG_APP        = "#F4F6FB"
COLOR_BG_SIDEBAR    = "#EEF1F6"
COLOR_BG_CHAT       = "#FFFFFF"

COLOR_ACCENT        = "#4F7DF3"
COLOR_ACCENT_HOVER  = "#3C68E0"

COLOR_BUBBLE_ME     = "#4F7DF3"
COLOR_BUBBLE_YOU    = "#E9EDF5"

COLOR_TEXT_ME       = "#FFFFFF"
COLOR_TEXT_YOU      = "#1F2937"

COLOR_INPUT_BG      = "#F1F3F7"
...
class Avatar(ctk.CTkFrame):
    def __init__(self, master, name, avatar_data=None, size=40, **kwargs):
        ...
        if image:
            ...
            fill_col = random.choice(colors)
            initial = name[0].upper() if name else "?"
            canvas.create_oval( 2, 2, size-2, size-2,fill=fill_col,outline="#FFFFFF",width=2)
...
class ChatBubble(ctk.CTkFrame):
    def __init__(self, master, text, is_me, timestamp, avatar_data=None, **kwargs):
        super().__init__(master, fg_color="transparent", **kwargs)
        ...
        row.pack(fill="x", padx=15, pady=4, anchor=anchor)

        # 👈 AVATAR CHO NGƯỜI KHÁC
        if not is_me:
            Avatar(row, "", avatar_data, size=28).pack(side="left", padx=(0, 6))
...
class ChatApp(ctk.CTk):
    def __init__(self, client_logic):
        ...
        self.my_avatar = Avatar(self.side_header, self.client.username, self.client.my_avatar_data, size=40)
        self.my_avatar.pack(side="left")
        ctk.CTkLabel(self.side_header, text="Chats", font=("Segoe UI", 26, "bold"), text_color=COLOR_ACCENT).pack(side="right", padx=12)
        ...
        # Chat Area
        self.chat_area = ctk.CTkFrame(self, fg_color=COLOR_BG_CHAT, corner_radius=0)
        self.chat_area.grid(row=0, column=1, sticky="nsew")
        self.chat_area.grid_rowconfigure(1, weight=1)
        self.chat_area.grid_columnconfigure(0, weight=1)
    ...
    font=("Segoe UI", 12)
        ...
        # ✅ SỬA: command gọi đúng method
        ctk.CTkButton( self.input_bar, text="➤",width=46,height=46, corner_radius=23,fg_color=COLOR_ACCENT,
